...
export const uploadPresaleImage = async (file: File, presaleAddress: string): Promise<string> => {
  try {
    // Validate file
    ...
    const { error } = await supabase.storage  // Remove 'data' from destructuring
      .from('presale-images')
...
export const getPresaleImage = async (presaleAddress: string): Promise<string | null> => {
  try {
