...
const ConnectWalletButton = () => {
  ...
  if (isConnected) {
    const isCorrectChainConnected = connectedChainId && allowedChainIds.includes(connectedChainId);
    ...
    return (
      <div className="flex flex-wrap items-center justify-end gap-x-2 gap-y-1">
        ...
        <span className="text-sm truncate max-w-[100px] hidden md:inline order-2" title={address || ""}>
          {address}
        </span>
        <Button variant="outline" size="sm" onClick={() => disconnect()} className="order-3">
          Disconnect
        </Button>
        ...
        {!isCorrectChainConnected && connectedChainId && (
          <p className="text-red-500 text-xs mt-1 w-full text-right order-5">
            Warning: Connected to unsupported network (ID: {connectedChainId}). Please switch to {allowedChainNames}.
          </p>
        )}
      </div>
    );
        ...
        <div className="grid gap-4 py-4">
          {readyConnectors.length > 0 ? (
              ...
              <Button
                ...
                onClick={async () => {
                  ...
                  try {
                    ...
                    console.error("Error during pre-connection check or connect:", err);
                    setChainError(`Failed to connect: ${err.message || 'Unknown error'}`);
                    setPendingConnectorId(null);
                  }
                }}
            ...
            <p className="text-center text-muted-foreground">
              No compatible wallet detected. Please ensure your browser wallet extension (e.g., MetaMask, Backpack) is installed and enabled.
            </p>
          )}
        </div>
        {chainError && <p className="text-red-500 text-sm mt-2">{chainError}</p>}
        {connectError && <p className="text-red-500 text-sm mt-2">{connectError.message}</p>}
      </DialogContent>
    </Dialog>
  );
};
